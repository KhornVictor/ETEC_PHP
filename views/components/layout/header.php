<header class="mb-4 flex justify-between w-full">
    <h1 class="m-0 text-[2rem] font-bold tracking-tight">Dashboard</h1>

    
    <div class="flex items-center gap-2.5">
        <div>
            <input class="w-full rounded-full border border-slate-200 bg-slate-50 px-4 py-2.5 text-sm outline-none ring-0 placeholder:text-slate-400 focus:border-slate-300" type="text" placeholder="Search request..." aria-label="Search">
        </div>
        <button class="grid h-9 w-9 place-items-center rounded-full border border-slate-200 bg-white text-sm font-bold text-slate-500" aria-label="Notifications">N</button>
        <button class="rounded-full bg-slate-900 px-4 py-2.5 text-sm font-semibold text-white">Upgrade plan</button>
        <img class="h-9 w-9 rounded-full object-cover" src="https://i.pravatar.cc/80?img=12" alt="Profile">
    </div>
</header>